            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <img src="<?php echo base_url('template/img/agent_ready.svg'); ?>" alt=""
                            class="kecil_img mx-auto d-block">
                        <h4 class="text-center kecil_kata mt-3 ">Agent Standby </h4>
                        <h6 class="text-center numberLog" id="agentStandby">0</h6>
                    </td>
                    ...
                    <td>
                        <img src="<?php echo base_url('template/img/asa.svg'); ?>" alt=""
                            class="kecil_img mx-auto d-block">
                        <h4 class="text-center kecil_kata mt-3 ">Response Time </h4>
                        <h6 class="text-center numberLog" id="responseTime">0</h6>
                    </td>
                </tr>
                <!-- <tr>
...
<div class="row" style="margin-top:2rem;">
    ...
    if ($this->session->userdata('privilege') == 0 || $this->session->userdata('privilege') == 9 || $this->session->userdata('privilege') == 1 || $this->session->userdata('privilege') == 2 || $this->session->userdata('privilege') == 4) {
        ?>
        <div class="col-md-4">
            <div id="incomingTicket" class="chart_wrap"></div>
        </div>
        <div class="col-md-4">
            <div id="handledTicket" class="chart_wrap"></div>
        </div>
        <div class="col-md-4">
            <div id="topAgent" class="chart_wrap"></div>
        </div>
        <div class="col-md-4">
            <div id="trendCategory" class="chart_wrap"></div>
        </div>
        <div class="col-md-4">
            <div id="trendActivation" class="chart_wrap"></div>
        </div>
        <div class="col-md-4">
            <div id="handlingServiceBy" class="chart_wrap"></div>
        </div>
        <?php
    }
    ?>
...
<script>
    $(document).ready(function () {
        ...
        getDasboard('<?php echo $this->session->userdata('privilege') == 0 || $this->session->userdata('privilege') == 9 || $this->session->userdata('privilege') == 1 || $this->session->userdata('privilege') == 2 ? '' : $this->session->userdata('area'); ?>');
    ...
    function getDasboard(val) {
        ...
        if (window.location.host == 'localhost') {
            urlAn = "https://localhost/sociolution/dashboard.act?type=analytic&" + add;
        } else {
            urlAn = "https://hakbesik.telkomcel.tl/dashboard.act?type=analytic&" + add;
        }
        ...
        $.ajax({
            ...
            success: function (json_raw) {
                ...
                for (var it in json.item) {
                    ...
                    if (it == 'responseTime' || it == 'averageHandling') {
                        $('#' + it).html($('#' + it).text() + ' <small>Minute</small>');
                    }
                }
                ...
                if ($this->session->userdata('privilege') == 0 || $this->session->userdata('privilege') == 9 || $this->session->userdata('privilege') == 1 || $this->session->userdata('privilege') == 2 || $this->session->userdata('privilege') == 4 || $this->session->userdata('privilege') == 5) {
                    ?>
                    Highcharts.chart('incomingTicket', {
                        chart: {
                        ...
                        series: [{
                            name: 'Total',
                            colorByPoint: true,
                            data: json.incomingTicket
                        }]
                    });
                    Highcharts.chart('handledTicket', {
                        chart: {
                            plotBackgroundColor: null,
                            plotBorderWidth: null,
                            plotShadow: false,
                    ...
                    Highcharts.chart('trendCategory', {
                        chart: {
                            plotBackgroundColor: null,
                            plotBorderWidth: null,
                            plotShadow: false,
                        ...
                        series: [{
                            name: 'Total',
                            colorByPoint: true,
                            data: json.trendCategory
                        }]
                    });
                    Highcharts.chart('trendActivation', {
                        chart: {
                            plotBackgroundColor: null,
                            plotBorderWidth: null,
                            plotShadow: false,
                        ...
                        series: [{
                            name: 'Total',
                            colorByPoint: true,
                            data: json.trendActivation
                        }]
                    });
                    Highcharts.chart('handlingServiceBy', {
                        chart: {
                            plotBackgroundColor: null,
                            plotBorderWidth: null,
                            plotShadow: false,
